...
Namespace Modules.LandingpageContact
...
#End Region
        ...
        Protected Sub submitbtn_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSubmit.Click
            Try
                Dim messagetext As String = "I am interested in"
                ...
                'If not (String.Compare(txtInterest.value, Interest,StringComparison.InvariantCultureIgnoreCase) == 0) Then
                '  message = txtInterest.value;
                'End If

                    message &= String.Format("First Name: {0}" & vbCrLf, txtName.Text)
                    message &= String.Format("Email Address: {0}" & vbCrLf, txtEmail.Text)
                message &= String.Format("Phone Number: {0}" & vbCrLf, txtPhone.Text)
                If (Not txtInterest.value = messagetext) Then
                    message &= String.Format("Message: {0}" & vbCrLf, txtInterest.value)
                End If
                'message &= String.Format("I am interested in: {0}" & vbCrLf, message)
